...
module.exports = {
	...
	aliases: ['k'],
	usage: 'kayıt @kardoxa/id isim/yaş',
    cooldown: 5,
	...
	execute(message, args, client) {
       ...
       if(!message.member.permissions.has("ADMINISTRATOR") && !message.member.roles.cache.has(ayarlar.yetkili)){
        message.channel.send(` ${message.author} İşlemi gerçekleştirmek için gerekli yetkiye sahip değilsin!`)
      ...
      let yaş = args[2]
       ...
       if(!isim || !yaş || !member){
        message.channel.send(` ${message.author}, Lütfen komutu uygun şekilde kullanın. \n Örnek kullanım: **${ayarlar.prefix}kayıt @Kardoxa/id İsim Yaş**`)
       
        return
    }
    if(isNaN(yaş)) return message.channel.send(` ${message.author}, Yaş sadece sayılardan oluşabilir.`)
   ...
   Kullanıcı daha önce kayıt olduğu isimleri görmek için \`isimler @kardoxa/id\` komutunu kullanabilirsin.
